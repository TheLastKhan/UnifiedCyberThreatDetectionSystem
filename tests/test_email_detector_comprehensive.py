...
class TestEmailPhishingDetector:
    ...
    def test_feature_extraction_structure(self, trained_detector):
        ...
        assert 'suspicious_sender' in features
        assert 'suspicious_subject' in features
...
class TestEmailDetectorPerformance:
    ...
    @pytest.mark.slow
    def test_training_time(self, benchmark):
        ...
        # Benchmark if pytest-benchmark installed
        try:
            result = benchmark(train_model)
            assert result.is_trained
        except:
            # Run normally if benchmark not available
